...
<body> 
    <nav class="navbar navbar-expand-lg navbar-dark bg-dark"> 
      ...
      border: 1px solid #dee2e6;"> 
             <h2 style="margin-top: 20px;">Modificar Obra</h2>
            <div class="form-group formulario" > 
              <form action="../controlador/controllerModificarObra.php" method="post" enctype="multipart/form-data">
              <input  name="parametros" type="text" class="form-control" hidden placeholder="Detalles" id="parametros" value="2">
              <input  name="idobra" type="text" class="form-control" hidden  id="idobra" value="<?php echo $_GET['id']?>" >
                <table class="formularioRegistrar">
                    ...
                    <tr>
                        ...
                        <td> <input required name="detalles" type="text" class="form-control" placeholder="Detalles" id="detalles"></td> 
                    </tr>
                    <tr> 
                        <td><h5>tecnica</h5></td> 
                    </tr> 
                    <tr>
                        <td> <input required name="tecnica"type="text" class="form-control" placeholder="tecnica" id="tecnica"></td> 
                    </tr>
                    <tr> 
                        <td><h5>Dimensiones</h5></td> 
                    </tr> 
                    <tr>
                        <td><input required name="altura" style="width: 45%; float: left;" type="number" class="form-control" placeholder="Altura (cm)" id="altura"> 
                            <input  required name="ancho" style="width: 45%; float: right; margin-left: 20px;" type="number" type="text" class="form-control" placeholder="Ancho (cm)" id="ancho"></td> 
                    </tr>
                    <tr>
                    <td><h5>Precio</h5></td> 
                    </tr> 
                    <tr>
                        <td><input name="precio"type="number" class="form-control" id="precio" placeholder="Precio (CLP)" required></td> 
                    </tr>
                    <tr> 
                      <td><h5>Stock</h5></td> 
                    </tr> 
                    <tr>
                        <td><input name="stock"type="number" class="form-control" id="stock" placeholder="Unidades disponibles" required></td> 
                    </tr>
                    ...
                    <tr>
                        <td><p><input name="imagenObra"class="form-control" type="file" id="formFile" ></textarea></p></td> 
                    </tr>  
                    
                    
                    <tr>
                        
                        <td><input type="submit" name="subir" style="background-color: #212529; border-radius: 0px ; border-color: #212529; color:white" class="btn btn-primary" value="modificar obra"></td>
                    </tr>
                </table>
...
$(document).ready(function(){
    ...
    $.ajax({
        url:'../controlador/controllerModificarObra.php',
        method:'POST',
        data:{parametros:parametros,id:id, iduser:iduser},
        success:function(data){
        // $('#reglon').html(data);
            var content= JSON.parse(data);
            //alert(content['titulo']);
            $("#titulo").val(content['titulo']);
            $("#exampleSelect1").val(content['categoria']);
            $("#detalles").val(content['detalles']);
            $("#tecnica").val(content['tecnica']);
            $("#altura").val(content['altura']);
            $("#ancho").val(content['ancho']);
            $("#precio").val(content['precio']);
            $("#stock").val(content['stock']);
            
            $("#exampleTextarea").val(content['sobreObra']);
        }
    });
    }
    obtener_datos();
    $(document).on("click", "#apruebo",function(){
      ...
      var id=$(this).data("id");
        ...
        $.ajax({
        url:"../controlador/controllerAdminModeradores.php",
        method:"POST",
